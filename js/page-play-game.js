var loadGamePlayHtml = function () {
    ...
    if (currentMatch != undefined) {
        ...
        mainHtml += "<tr>" +
            ...
            "<td class='class-td-button' onclick='loadFakeFrame();'>约赛情报</td>" +
...
var startThisMatch = function () {
    ...
    while (halfTimeSeconds > eclipseSeconds) {
        ...
        if (getTeamPowerInPlay(homeTeamPlayers) * getRandomInteger(100) / getTeamPowerInPlay(awayTeamPlayers) > 50) {
            ...
            teamAttack(eclipseSeconds, homeTeamGk, awayTeamGk, homeLineUpTeamPlayers, homeLineUpBackupTeamPlayers, awayLineUpTeamPlayers, awayLineUpBackupTeamPlayers, currentMatch, 0, userTeam);
        } else {
            logMatch(getEclipseMinutesAndSeconds(eclipseSeconds) + " 客队发起进攻<br/>");
            teamAttack(eclipseSeconds, awayTeamGk, homeTeamGk, awayLineUpTeamPlayers, awayLineUpBackupTeamPlayers, homeLineUpTeamPlayers, homeLineUpBackupTeamPlayers, currentMatch, 1, userTeam);
        }
    }
    logMatch("上半场结束<br/>");
...
var teamAttack = function (eclipseSeconds, attackTeamGk, defenceTeamGk, homeLineUpTeamPlayers, homeLineUpBackupTeamPlayers, awayLineUpTeamPlayers, awayLineUpBackupTeamPlayers, currentMatch, attackWay, userTeam) {
    ...
    if (startAttackPlayer != undefined) {
        ...
        if (startDefencePlayer != undefined) {
            ...
            while (attackPlayers.length > 0 && defencePlayers.length > 0) {
                ...
                //过人VS逼抢
                if (getRandomInteger(attackPlayer.dribble) + plusHappyPointsInMatchAttack(attackPlayer, defencePlayer, match) >= getRandomInteger(defencePlayer.closingDown)) {
                    eclipseSeconds += 1;
                    ...
                    if (getRandomInteger(attackPlayer.attack) + plusHappyPointsInMatchAttack(attackPlayer, defencePlayer, match) >= getRandomInteger(defencePlayer.defence)) {
                        ...
                        logMatch(getEclipseMinutesAndSeconds(eclipseSeconds) + " <span class='class-span-play-game-attack-player' playerId='" + attackPlayer.id + "' teamId='" + (attackWay == 0 ? match.homeTeamId : match.awayTeamId) +
                            ...
                            "]</span> 试图传球！<br/>");
                        //传球VS拦截
                        if (getRandomInteger(attackPlayer.pass) + plusHappyPointsInMatchAttack(attackPlayer, defencePlayer, match) >= getRandomInteger(defencePlayer.intercept)) {
                            eclipseSeconds += 1;
                            logMatch(
        ...
        if (attackPlayers.length == 0 && keepAttackPlayer == undefined) {
            ...
            if (attackPlayer == undefined) {
                ...
                while (attackPlayer != undefined) {
                    ...
                    //射门VS封堵
                    if ((getRandomInteger(attackPlayer.shoot) + plusHappyPointsInMatchAttack(attackPlayer, defencePlayer, match)) > getRandomInteger(defenceTeamGk.block)) {
                        eclipseSeconds += 1;
...
var getHomeOrAwayTeamLineUpHtml = function (matchTeam, teamIsHomeOrAway) {
    ...
    if (teamIsHomeOrAway == "客") {
        mainHtml += "<tr><td class='" + (userTeam.config.teamId == matchTeam.teamInfo.id ? "class-td-button" : "") + "'>前锋</td><td class='class-td-lineup'>" + getPlayerLineUpHtml(fw, matchTeam) + "</td></tr>";
        mainHtml += "<tr><td class='" + (userTeam.config.teamId == matchTeam.teamInfo.id ? "class-td-button" : "") + "'>中场</td><td class='class-td-lineup'>" + getPlayerLineUpHtml(mf, matchTeam) + "</td></tr>";
        mainHtml += "<tr><td class='" + (userTeam.config.teamId == matchTeam.teamInfo.id ? "class-td-button" : "") + "'>后卫</td><td class='class-td-lineup'>" + getPlayerLineUpHtml(cb, matchTeam) + "</td></tr>";
        mainHtml += "<tr><td class='" + (userTeam.config.teamId == matchTeam.teamInfo.id ? "class-td-button" : "") + "'>门将</td><td class='class-td-lineup'>" + getPlayerLineUpHtml(gk, matchTeam) + "</td></tr>";
            ...
            if (homeTeam != undefined && awayTeam != undefined) {
                ...
                if (myTeam.teamInfo.id == homeTeam.teamInfo.id) {
                    ...
                    sendMailToTeam("球迷买票费用入账了", "<table>" +
                        ...
                        "<tr><td>实际入账虚拟金额</td><td>" + match.homeFansCount * homeTeam.teamStadium.ticketPrice + "</td></tr>" +
                        "</table>");
                    ...
                    sendMailToTeam("球迷买票费用入账了", "<table>" +
                        ...
                        "<tr><td>实际入账虚拟金额</td><td>" + match.awayFansCount * homeTeam.teamStadium.ticketPrice + "</td></tr>" +
                        "</table>");
                }
            }
        }
...
var sponsorTeam = function (currentTeam) {
    ...
    if (currentTeam.sponsors.length - continueSponsorCount <= 30) {
        ...
        sponsor.name = getSingleRandomLastName() + getSingleRandomLastName() + "有限公司";
    ...
    for (var i = 0; i < teamPlayers.length; i++) {
        ...
        if (teamPlayer.girlIndex != undefined) {
            ...
            if (teamPlayer.money > 2000000) {
                var carIndex = PICTURE_COUNT_GIFT + getRandomInteger(teamPlayer.money / 1000000);
                ...
                girl.cars.push(userTeam.shop.products[carIndex]);
            }
            //1000万送房子
            if (teamPlayer.money > 10000000) {
                var houseIndex = PICTURE_COUNT_GIFT + PICTURE_COUNT_CAR + getRandomInteger(teamPlayer.money / 5000000);
                userTeam.shop.products[houseIndex].stock -= 1;
                var money = userTeam.shop.products[houseIndex].money;
                teamPlayer.money -= money;
                girl = userTeam.girls[teamPlayer.girlIndex];
                increaseHappy(userTeam.girls[teamPlayer.girlIndex], teamPlayer, money / 20000);
                girl.houses.push(userTeam.shop.products[houseIndex]);
            }
            if (girl != undefined) {
                ...
                if (girlHappy > girl.dateHappy) {
                    ...
                    if (getRandomInteger(100) < 100) {
                        //约炮
                        if (girl.sexMans[teamPlayer.id] == undefined) {
                            girl.sexMans[teamPlayer.id] = 1;
                        } else {
                            girl.sexMans[teamPlayer.id] += 1;
                        }
...
var getFake = function (season, games, level) {
    var fake = {};
    fake.id = "FAK" + getId();
    fake.name = getSingleRandomLastName() + getSingleRandomLastName() + "有限公司";
    fake.picture = getRandomInteger(PICTURE_COUNT_MANAGER);
    fake.sponsorRelation = 1 + getRandomInteger(10);
    fake.money = level * (5666666 + getRandomInteger(86666666));
    fake.notMeetRequirementLoseMoney = level * (5666666 + getRandomInteger(86666666));
    fake.risk = 1 + getRandomInteger(100);
    ...
    fake.breakLoseMoney = level * (5666666 + getRandomInteger(76666666));
    fake.games = games;
    fake.season = season;
